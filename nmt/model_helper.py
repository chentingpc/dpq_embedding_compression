...
def create_emb_for_encoder_and_decoder(share_vocab,
    ...
    Raises:
      ValueError: if use share_vocab but source and target have different vocab
        size.
    """
    assert share_vocab == False
    assert num_enc_partitions == 0
    assert num_dec_partitions == 0
    assert src_embed_file == None or src_embed_file == ''
    assert tgt_embed_file == None or tgt_embed_file == ''
    assert use_char_encode == False
    # import pdb
    # pdb.set_trace()
    use_kdq_to_control = True  # controlled by kdq_type
