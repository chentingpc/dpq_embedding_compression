...
import os
import sys
parent_path = "/".join(os.getcwd().split('/'))
sys.path.append(os.path.join(parent_path, "core"))
sys.path.append(os.path.join(parent_path, "kdq/core"))
    ...
    def _build_encoder_from_sequence(self, hparams, sequence, sequence_length):
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            if self.embedding_encoder is not None:
                ...
                size = hparams.num_units
                if FLAGS.kdq_type == "none":
                    inputs = full_embed(sequence, vocab_size, size)
                else:
                    kdq_hparam = KDQhparam(
                        K=FLAGS.K, D=FLAGS.D,
                        kdq_type=FLAGS.kdq_type,
                        kdq_d_in=FLAGS.kdq_d_in,
                        kdq_share_subspace=FLAGS.kdq_share_subspace,
                        additive_quantization=FLAGS.additive_quantization)
                    inputs = kdq_embed(
                        sequence, vocab_size, size, kdq_hparam, is_training)
                self.encoder_emb_inp = inputs
